...
# Aggregate by retailer name and time period

# Monthly by retailer and size
monthly_by_retailer <- transfers %>%
  filter(stat_shortcut == "M57A", !is.na(FRMP_Retailer_name)) %>%
  group_by(FRMP_Retailer_name, YearMonth, stat_date = floor_date(stat_date, "month"), FRMP_Retailer_size) %>%
  summarise(
    transfers = sum(stat_value, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
# Quarterly by retailer and size
quarterly_by_retailer <- transfers %>%
  filter(stat_shortcut == "M57A", !is.na(FRMP_Retailer_name)) %>%
  group_by(FRMP_Retailer_name, FinYearQuarter, FRMP_Retailer_size) %>%
  summarise(
    transfers = sum(stat_value, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
# Financial year by retailer and size
finyear_by_retailer <- transfers %>%
  filter(stat_shortcut == "M57A", !is.na(FRMP_Retailer_name)) %>%
  group_by(FRMP_Retailer_name, FinYear, FRMP_Retailer_size) %>%
  summarise(
    transfers = sum(stat_value, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
# For each retailer, add FY + Quarterly + Monthly traces (12 traces per retailer)
for (retailer in retailer_names) {
  is_visible <- (retailer == first_retailer)
  
  # Financial Year traces (4 sizes)
  for (size in retailer_sizes) {
    data_subset <- finyear_by_retailer %>% 
      filter(FRMP_Retailer_name == retailer, FRMP_Retailer_size == size)
  ...
  # Quarterly traces (4 sizes, hidden initially)
  for (size in retailer_sizes) {
  ...
  # Monthly traces (4 sizes, hidden initially)
  for (size in retailer_sizes) {
...
# Each retailer has 12 traces (4 sizes × 3 time periods)
retailer_buttons <- lapply(seq_along(retailer_names), function(i) {
  retailer <- retailer_names[i]
  
  # Calculate which traces to show (first 4 for FY view)
  start_idx <- (i - 1) * 12 + 1
  visible_vector <- rep(FALSE, length(retailer_names) * 12)
  visible_vector[start_idx:(start_idx + 3)] <- TRUE  # Show FY traces
...
# These work relative to current selected retailer
time_buttons <- list(
  list(
    label = "Financial Year",
    method = "restyle",
    args = list(
      "visible",
      lapply(1:(length(retailer_names) * 12), function(i) {
        retailer_idx <- ceiling(i / 12)
        trace_in_retailer <- ((i - 1) %% 12) + 1
        # Show if: this retailer is visible AND this is a FY trace (1-4)
        trace_in_retailer <= 4
      })
    )
  ),
  list(
    ...
    args = list(
      ...
      lapply(1:(length(retailer_names) * 12), function(i) {
        trace_in_retailer <- ((i - 1) %% 12) + 1
        # Show if: this is a Quarterly trace (5-8)
        trace_in_retailer >= 5 && trace_in_retailer <= 8
      })
    )
  ),
  list(
    ...
    args = list(
      ...
      lapply(1:(length(retailer_names) * 12), function(i) {
        trace_in_retailer <- ((i - 1) %% 12) + 1
        # Show if: this is a Monthly trace (9-12)
        trace_in_retailer >= 9
      })
    )
  )
...
p_with_filter <- p_with_filter %>%
  layout(
    title = list(
      text = paste("Transfer Trends -", first_retailer),
      font = font_title
    ),
    xaxis = list(
      title = "Financial Year",
...
```{r insights-2}
# Most common transfer type
top_type <- finyear_by_type %>%
...
latest_month_types <- monthly_by_type %>%
  filter(stat_date == max(stat_date)) %>%
  arrange(desc(transfers))

cat(paste0(
  "- Most common transfer type overall: ", top_type$transfer_type, 
  " (", format(top_type$total, big.mark = ","), " total transfers)\n",
  "- Latest month breakdown:\n"
))

for (i in 1:nrow(latest_month_types)) {
  cat(paste0(
    "  - ", latest_month_types$transfer_type[i], ": ",
    format(latest_month_types$transfers[i], big.mark = ","), 
    " (", round(latest_month_types$transfers[i] / sum(latest_month_types$transfers) * 100, 1), "%)\n"
  ))
}
```
...
```{r insights-3}
# Breakdown by type and size for latest FY
for (ttype in transfer_types) {
  type_data <- finyear_type_size %>%
    filter(FinYear == max(FinYear), transfer_type == ttype) %>%
    arrange(desc(transfers))
  
  if (nrow(type_data) > 0) {
    cat(paste0("\n", ttype, " (FY ", max(finyear_type_size$FinYear), "):\n"))
    for (i in 1:nrow(type_data)) {
      cat(paste0(
        "  - ", type_data$FRMP_Retailer_size[i], ": ",
        format(type_data$transfers[i], big.mark = ","), " transfers\n"
      ))
    }
  }
}
