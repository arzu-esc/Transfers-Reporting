...
```{r load-data}
transfers_raw <- readRDS(here("01_monthly_scripts/02_data", "transfers_raw.rds"))
```
...
**Data Context:** This report includes electricity small customers who (1) switch from one retailer to another while remaining at the same premises (2) move into a different property and chose a different retailer, and (3) transfer due to another reason. Small customers are defined as those consuming <= 160MWh.
...
latest_month_test <- transfers %>%
  filter(stat_shortcut == "M57A", 
         !is.na(FRMP_Retailer_name),
         !is.na(NEWFRMP_Retailer_name),
         is_latest_month == "TRUE") %>%
  group_by(FRMP_Retailer_name, NEWFRMP_Retailer_name, FRMP_Retailer_size, NEWFRMP_Retailer_size) %>%
  summarise(
    transfers = sum(stat_value, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
divider_fy <- list(label = "─── FINANCIAL YEAR ───", method = "skip")
divider_q <- list(label = "─── QUARTERLY ───", method = "skip")
divider_m <- list(label = "─── MONTHLY ───", method = "skip")
...
```{r move-in-prep}
# Get the most recent date and calculate 15 months ago
max_date <- max(transfers$stat_date, na.rm = TRUE)
months_ago_15 <- max_date %m-% months(15)

# Filter for Move-in transfers in last 15 months
...
move_in_data <- transfers %>%
  filter(
    ...
    stat_date >= months_ago_15,
    stat_date <= max_date,
    !is.na(NEWFRMP_Retailer_name)
  ) %>%
  mutate(year_month = floor_date(stat_date, "month"))

# Get top 10 retailers by total move-in transfers over last 15 months
...
p_move_in <- p_move_in %>%
  layout(
    margin = list(t = 80),
    title = list(
      text = "Top 5 Retailers - Move-In Transfers (Last 15 Months)",
      font = font_title
    ),
    xaxis = list(
      title = "Month",
...
## 3.1 Transfers OUT by Retailer for the past 15 months
...
# Get top 10 retailers by total transfers out in last 15 months
top_retailers_out <- transfers_out_15m %>%
...
# Create line chart for transfers OUT (last 15 months)
...
p_transfers_out <- p_transfers_out %>%
  layout(
    margin = list(t = 80),
    title = list(
      text = "Top 10 Retailers - Transfers OUT (Customer Losses) - Last 15 Months",
      font = font_title
    ),
    xaxis = list(
      title = "Month",
...
# Top losers in last 15 months
latest_fy_out <- transfers_out_15m %>%
  filter(FRMP_Retailer_name %in% top_retailers_out) %>%
  group_by(FRMP_Retailer_name) %>%
  summarise(total_out = sum(stat_value, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(total_out))
...
# Top losers in last 15 months
cat(paste0(
  "<b>Top retailers losing customers (Last 15 Months: ",
  format(months_ago_15, "%B %Y"), " to ", format(max_date, "%B %Y"), "):</b><br><br>"
))
...
## 3.3 Transfers IN by Retailer for the past 15 months

```{r chart-5-prep}
# Transfers IN = where retailer is NEWFRMP (destination)
# Filter for last 15 months
transfers_in_15m <- transfers %>%
  filter(
    stat_shortcut == "M57A",
    stat_date >= months_ago_15,
    stat_date <= max_date,
...
# Get top 10 retailers by total transfers in in last 15 months
top_retailers_in <- transfers_in_15m %>%
  group_by(NEWFRMP_Retailer_name) %>%
...
# Create line chart for transfers IN (last 15 months)
...
p_transfers_in <- p_transfers_in %>%
  layout(
    margin = list(t=80),
    title = list(
      text = "Top 10 Retailers - Transfers IN (Customer Gains) - Last 15 Months",
      font = font_title
    ),
    xaxis = list(
      title = "Month",
...
# Top gainers in last 15 months
latest_fy_in <- transfers_in_15m %>%
  filter(NEWFRMP_Retailer_name %in% top_retailers_in) %>%
  group_by(NEWFRMP_Retailer_name) %>%
  summarise(total_in = sum(stat_value, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(total_in))
...
cat(paste0(
  "<b>Top retailers gaining customers (Last 15 Months: ",
  format(months_ago_15, "%B %Y"), " to ", format(max_date, "%B %Y"), "):</b><br><br>"
))
...
<p style="margin-top: 20px; font-style: italic; color: #666;">📊 <b>Note:</b> Refer to the summary table in section 3.2 above for detailed transfer flows between all retailers.</p>

## 3.5 Retailer Transfer Metrics Summary

### Select Month {.tabset}

```{r section-3-5-prep}
# Get last 15 months with M57A and M71 data
available_months_metrics <- transfers %>%
  filter(stat_shortcut %in% c("M57A", "M71")) %>%
  distinct(YearMonth, stat_date) %>%
  arrange(desc(stat_date)) %>%
  distinct(YearMonth, .keep_all = TRUE) %>%
  slice(1:15)

# Pre-calculate M71 data for all months (for efficiency)
m71_all <- transfers %>%
  filter(stat_shortcut == "M71", !is.na(FRMP_Retailer_name)) %>%
  group_by(YearMonth, FRMP_Retailer_name) %>%
  summarise(active_nmis = sum(stat_value, na.rm = TRUE), .groups = "drop")

# Pre-calculate M57A transfers OUT for all months
transfers_out_all <- transfers %>%
  filter(stat_shortcut == "M57A", !is.na(FRMP_Retailer_name)) %>%
  group_by(YearMonth, retailer = FRMP_Retailer_name) %>%
  summarise(transfers_out = sum(stat_value, na.rm = TRUE), .groups = "drop")

# Pre-calculate M57A transfers IN for all months
transfers_in_all <- transfers %>%
  filter(stat_shortcut == "M57A", !is.na(NEWFRMP_Retailer_name)) %>%
  group_by(YearMonth, retailer = NEWFRMP_Retailer_name) %>%
  summarise(transfers_in = sum(stat_value, na.rm = TRUE), .groups = "drop")

# Initialize the list to store metrics
metrics_list <- list()

# Pre-calculate metrics for all 15 months
for (i in 1:nrow(available_months_metrics)) {
  month_date <- available_months_metrics$stat_date[i]
  month_str <- format(as.Date(month_date), "%Y-%m")
  prev_month_str <- format(as.Date(month_date) %m-% months(1), "%Y-%m")
  
  # Get pre-calculated data for this month
  m71_month <- m71_all %>% filter(YearMonth == prev_month_str)
  out_month <- transfers_out_all %>% filter(YearMonth == month_str)
  in_month <- transfers_in_all %>% filter(YearMonth == month_str)
  
  # Combine metrics
  metrics <- out_month %>%
    full_join(in_month, by = "retailer") %>%
    left_join(m71_month, by = c("retailer" = "FRMP_Retailer_name")) %>%
    mutate(
      transfers_out = replace_na(transfers_out, 0),
      transfers_in = replace_na(transfers_in, 0),
      active_nmis = replace_na(active_nmis, 0),
      net_transfer = transfers_in - transfers_out,
      churn_rate = ifelse(active_nmis > 0, (transfers_out / active_nmis) * 100, 0),
      acquisition_rate = ifelse(active_nmis > 0, (transfers_in / active_nmis) * 100, 0),
      net_rate = ifelse(active_nmis > 0, (net_transfer / active_nmis) * 100, 0)
    ) %>%
    select(retailer, transfers_out, transfers_in, active_nmis, net_transfer, 
           churn_rate, acquisition_rate, net_rate) %>%
    arrange(desc(abs(net_transfer)))
  
  metrics_list[[month_str]] <- metrics
}
```

```{r section-3-5-display, echo=FALSE, results='asis'}
retailer_order <- transfers %>%
  pull(FRMP_Retailer_name) %>%
  unique() %>%
  na.omit() %>%
  sort()

for (i in seq_len(nrow(available_months_metrics))) {

  month_date  <- available_months_metrics$stat_date[i]
  month_str   <- format(as.Date(month_date), "%Y-%m")
  month_label <- format(as.Date(month_date), "%B %Y")

  # ---- Tab header (THIS creates the tab) ----
  cat("#### ", month_label, "\n\n")

  metrics_data <- metrics_list[[month_str]]

  if (!is.null(metrics_data) && nrow(metrics_data) > 0) {

    metrics_data %>%
      mutate(
        retailer = factor(retailer, levels = retailer_order)
      ) %>%
      arrange(retailer) %>%
      transmute(
        Retailer = retailer,
        `Active NMIs` = format(active_nmis, big.mark = ","),
        `Transfers IN` = format(transfers_in, big.mark = ","),
        `Transfers OUT` = format(transfers_out, big.mark = ","),
        `Net Transfer` = format(net_transfer, big.mark = ","),
        `Churn Rate (%)` = sprintf("%.2f%%", churn_rate),
        `Acquisition Rate (%)` = sprintf("%.2f%%", acquisition_rate),
        `Net Rate (%)` = sprintf("%.2f%%", net_rate)
      ) %>%
      kable(
        format = "html",
        escape = FALSE,
...
```{r section-4-1-sankey-only, echo=FALSE, results='asis'}
library(htmltools)

for (i in seq_len(nrow(available_months))) {
  ...
  # ---- Render Sankey ----
  sankey_plot <- create_sankey(month_date)

  if (!is.null(sankey_plot)) {
    print(tagList(sankey_plot))
  }
...
p_in_out <- p_in_out %>%
  layout(
    margin = list(t = 130),
    title = list(
      text = paste("Transfers IN vs OUT –", latest_month),
      font = font_title,
      x = 0.5,
      xanchor = "center"
    ),
